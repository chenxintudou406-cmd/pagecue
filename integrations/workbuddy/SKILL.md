...
9. 知识提醒创建成功后只回复：`创建成功`、提醒类型、标题、提交人、关键词及 OR/AND、强度。操作提醒创建成功后只回复：`创建成功`、标题、提交人、关键词及 OR/AND、强度、有效期；操作提醒不要再显示“提醒类型”。两类回复都不要追加页面范围、投放对象、冷却时间、链接数量、默认值、同步状态或提醒 ID。失败或重复时仍完整回复接口提供的具体原因。
